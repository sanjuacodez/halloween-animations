...
(function($) {
    ...
    class HalloweenEffects {
        constructor() {
            this.settings = wpHalloween.settings || {};
        ...
        initializeBats() {
            if (!this.settings.bats_enabled) return;

            const batsContainer = $('#halloween-bats');
            if (!batsContainer.length) return;

            const batCount = parseInt(this.settings.bats_count) || 5;
            ...
            this.batSystem = new HalloweenBatSystem(batsContainer, {
                image: wpHalloween.pluginUrl + 'assets/images/bats.png',
                amount: batCount,
                speed: speed,
                width: 35,
                height: 20,
        ...
        getSpeedValue(type) {
            const speed = this.settings[type + '_speed'] || 'medium';
            switch (speed) {
                case 'slow': return 10;
                case 'fast': return 35;
                case 'medium':
        ...
        initializeGhosts() {
            if (!this.settings.ghosts_enabled) return;

            const ghostsContainer = $('#halloween-ghosts');
            if (!ghostsContainer.length) return;

            const ghostCount = parseInt(this.settings.ghosts_count) || 3;
            const speed = this.settings.ghosts_speed || 'medium';
        ...
        initializePumpkin() {
            if (!this.settings.pumpkin_enabled) return;

            const pumpkinContainer = $('#halloween-pumpkin');
            if (!pumpkinContainer.length) return;

            const speed = this.settings.pumpkin_speed || 'medium';
        ...
        initializeLeaves() {
            if (!this.settings.leaves_enabled) return;

            const leavesContainer = $('#halloween-leaves');
            if (!leavesContainer.length) return;

            const leafCount = parseInt(this.settings.leaves_count) || 10;
        ...
        initializeSpiders() {
            if (!this.settings.spiders_enabled) return;

            const spidersContainer = $('#halloween-spiders');
            if (!spidersContainer.length) return;

            const spiderCount = parseInt(this.settings.spiders_count) || 2;
